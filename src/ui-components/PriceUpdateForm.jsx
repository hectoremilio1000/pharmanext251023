...
export default function PriceUpdateForm(props) {
  const {
    id: idProp,
    price: priceModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = priceRecord
      ? { ...initialValues, ...priceRecord }
      : initialValues;
    setCurrencyIso(cleanValues.currencyIso);
    setFormattedValue(cleanValues.formattedValue);
    setPriceType(cleanValues.priceType);
    setValue(cleanValues.value);
    setErrors({});
  };
  const [priceRecord, setPriceRecord] = React.useState(priceModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Price, idProp)
        : priceModelProp;
      setPriceRecord(record);
    };
    queryData();
  }, [idProp, priceModelProp]);
  React.useEffect(resetStateValues, [priceRecord]);
  const validations = {
    currencyIso: [],
    formattedValue: [],
    priceType: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Price.copyOf(priceRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "PriceUpdateForm")}
      {...rest}
    >
      <TextField
        label="Currency iso"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || priceModelProp)}
